...
//
import { existsSync, mkdirSync, readdirSync, statSync, readFileSync, copyFileSync, rmSync } from "fs";
...
import { scanString, extractPrintableStrings, type SecretMatch } from "./secrets.ts";
import { detectPinningSignatures, generateCustomHookStub, type PinningSignature } from "./unpinning.ts";
...
function parseArgs(argv: string[]): ExtractOptions {
  const options: ExtractOptions = { device: "target", jadx: true, noScan: false };
  for (const arg of argv.slice(2)) {
    if (arg === "--help" || arg === "-h") { printHelp(); process.exit(0); }
    if (arg === "--no-scan") { options.noScan = true; continue; }
    if (arg === "--no-jadx") { options.jadx = false; continue; }
    const match = arg.match(/^--([^=]+)(?:=(.*))?$/);
    if (!match) continue;
    const [, key, value] = match;
    switch (key) {
      case "package": options.package = value; break;
      case "apk":     options.apk = value; break;
      case "device":  options.device = value ?? options.device; break;
...
const TEXT_EXTENSIONS = new Set([".json", ".txt", ".properties", ".java", ".kt", ".js", ".html", ".yml", ".yaml", ".cfg", ".ini", ".md"]);
const SKIP_EXTENSIONS = new Set([".png", ".jpg", ".jpeg", ".gif", ".webp", ".ttf", ".otf", ".mp3", ".mp4", ".ogg", ".zip", ".webm"]);
...
function writeReport(
  ...
  if (info?.versionName) lines.push(`Version: ${info.versionName}${info.versionCode ? ` (code ${info.versionCode})` : ""}`);
  lines.push("");
  lines.push("## Files");
  lines.push("");
  for (const p of pulledApks) lines.push(`- Pulled APK: \`${relative(outDir, p)}\``);
  lines.push(`- Extracted contents: \`${relative(outDir, extractedDir)}\``);
  if (decompiledDir) lines.push(`- Decompiled sources (jadx): \`${relative(outDir, decompiledDir)}\``);
  else lines.push(`- Decompiled sources: not generated (jadx not found on PATH, or --no-jadx passed)`);
  lines.push("");

  lines.push("## Pinning detection");
  lines.push("");
  if (signatures.length === 0) {
    lines.push("No known pinning-library fingerprints found in the extracted dex/resources.");
  } else {
    lines.push("| Technique | Status | Evidence |");
    lines.push("|---|---|---|");
    for (const s of signatures) lines.push(`| ${s.label} | ${s.status} | ${s.evidence} |`);
    lines.push("");
    lines.push("Status meanings:");
    lines.push("- `covered` — the default HTTPToolkit suite (`scripts/unpinning/`) already handles this.");
    lines.push("- `verify-manually` — partially handled; capture traffic and confirm it's actually unpinned.");
    lines.push("- `needs-custom` — not generically coverable; a starter hook was generated (see below) if one didn't already exist.");
  }
  if (generatedStub) {
    lines.push("");
    lines.push(`Generated a starter custom hook: \`${relative(outDir, generatedStub)}\`. Fill in its TODOs, then re-run \`bun run.ts --package=${pkg}\`.`);
  }
  lines.push("");
  ...
  lines.push("");
  if (matches.length === 0) {
    lines.push("No matches from the built-in secret-pattern list. This does not mean the app has none — it means the static patterns in `src/secrets.ts` didn't match anything in the extracted/decompiled text.");
  } else {
    ...
    lines.push("`generic-credential-like` matches are a broad heuristic (any `key: \"value\"` shape near words like token/secret/password) — treat them as leads to verify, not confirmed secrets.");
  }
  lines.push("");

  lines.push("## Suggested next steps");
  lines.push("");
  lines.push(`- Capture live traffic alongside these static findings: \`bun run run -- --package=${pkg}\``);
  if (matches.some(m => m.category === "urls" || m.category === "ips")) {
    lines.push("- Check whether any hardcoded hosts/IPs above are internal/staging endpoints reachable from this network.");
  }
  lines.push("- This report and everything under `testing/` is git-ignored — it stays local unless you copy it out.");
...
async function main(): Promise<void> {
  ...
  if (!options.package && !options.apk) fail("--package=<pkg> or --apk=<path> is required.");
  ...
  if (!pkg && options.apk) {
    ...
    if (apkInfo) {
      ...
      pkg = basename(options.apk).replace(/\.apk$/i, "").replace(/[^A-Za-z0-9._-]/g, "_");
      log.warn(`Could not read the real package name (need aapt/aapt2 or apkanalyzer — run \`bun run init -- --install-sdk\`). Using '${pkg}' as a folder name instead.`);
    }
  }
  ...
  if (options.apk) {
    ...
    const serial = options.device === "target" ? cfg.targetSerial
      : options.device === "source" ? cfg.sourceSerial
      : options.device;
    const adbTool = findAdb(platform, cfg.sdkRoot, serial);
    adbTool.startServer();
    const adbRun = (...args: string[]) => run(adbTool.exePath, ["-s", serial, ...args]);
    const remotePaths = packageApkPaths(adbRun, pkg);
    if (remotePaths.length === 0) fail(`Package '${pkg}' is not installed on device '${serial}'.`);
  ...
  log.good(`Extracted ${pulledApks.length} APK(s) into ${relative(labRoot, extractedDir)}`);

  let decompiledOut: string | null = null;
  if (options.jadx) {
    log.step("Decompile");
    const jadx = which("jadx") ?? which("jadx.bat");
    if (!jadx) {
      log.info("jadx not found on PATH — using raw-strings scan only. Install jadx for deeper Java-level results: https://github.com/skylot/jadx");
    } else {
      if (existsSync(decompiledDir)) rmSync(decompiledDir, { recursive: true, force: true });
      const base = pulledApks[0];
      log.info(`Running jadx on ${basename(base)}… (this can take a while for large apps)`);
      const r = run(jadx, ["-d", decompiledDir, "--no-res", base]);
      if (r.ok || existsSync(decompiledDir)) {
        decompiledOut = decompiledDir;
        log.good(`Decompiled sources written to ${relative(labRoot, decompiledDir)}`);
      } else {
        log.info(`jadx failed to produce output (${r.stderr.trim().slice(0, 200) || "unknown error"}) — continuing without decompiled sources.`);
      }
    }
  }
  ...
  if (!options.noScan) {
    log.step("Scan");
    matches = scanTree([extractedDir, ...(decompiledOut ? [decompiledOut] : [])]);
    log.good(`Found ${matches.length} unique match(es) from the built-in secret patterns.`);

    signatures = detectPinningSignatures(extractedDir);
    if (signatures.length > 0) {
      log.info(`Pinning signatures detected: ${signatures.map(s => `${s.label} (${s.status})`).join(", ")}`);
      generatedStub = generateCustomHookStub(labRoot, pkg, signatures);
      if (generatedStub) log.warn(`Generated starter custom hook: ${relative(labRoot, generatedStub)} — fill in its TODOs before relying on it.`);
    } else {
      log.info("No known pinning-library fingerprints found.");
    }
  }

  writeReport(outDir, pkg, apkInfo, pulledApks, extractedDir, decompiledOut, matches, signatures, generatedStub);
