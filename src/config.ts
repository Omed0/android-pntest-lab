...
export const DEFAULTS = {
  ...
  // ── Root ──────────────────────────────────────────────────────────────────
  /**
   * Plain `google_apis` images support `adb root` directly, so the target
   * does not need the Play-Store-image Magisk/rootAVD path by default.
   * Enable explicitly with --magisk-root only when a test requires Magisk.
   */
  magiskRoot: true,
} as const;
...
export function loadConfig(
  ...
  return {
    ...
    sourceImagePackage: str(
      "source-image-package",
      DEFAULTS.sourceImagePackage,
    ),
    sourceDeviceProfile: str(
      "source-device-profile",
      DEFAULTS.sourceDeviceProfile,
    ),
    ...
    sdkRoot,
    cmdlineToolsVersion: str(
      "cmdline-tools-version",
      DEFAULTS.cmdlineToolsVersion,
    ),
    ...
    proxyEnabled:
      bool("proxy", DEFAULTS.proxyEnabled) &&
      bool("burp", DEFAULTS.proxyEnabled),
    proxyTool:
      strAlias(["proxy-tool"], DEFAULTS.proxyTool) === "other"
        ? "other"
        : "burp",

    emulatorBootTimeoutSec: num(
      "boot-timeout",
      DEFAULTS.emulatorBootTimeoutSec,
    ),
...
AVD options (default → env var → flag)
  ...
  --source-serial=<id>      Optional source serial [LAB_SOURCE_SERIAL]
  --source-avd=<name>       Optional source AVD [LAB_SOURCE_AVD]
  --source-image-package=<p> Optional source image package [LAB_SOURCE_IMAGE_PACKAGE]
...
Root options
  --no-magisk-root         Skip patching the target's ramdisk with real
                           Magisk via rootAVD. Off by default because the
                           default google_apis image supports plain adb-root.
                           Enable with --magisk-root only when needed.
...
export function printConfig(cfg: LabConfig): void {
  ...
  console.log(
    "  Frida:  ",
    cfg.fridaVersion === "auto" ? "auto (match host)" : cfg.fridaVersion,
  );
  console.log(
    "  Proxy:  ",
    `${cfg.burpHost}:${cfg.burpPort}`,
    "(Burp by default)",
  );
  console.log(
    "  GPU:    ",
    cfg.gpuMode,
    cfg.portable ? " · portable mode" : "",
  );
}
